...
export default class PasswordController {
  async forgot({ request, response }: HttpContext) {
    ...
    await Mail.send((message) => {
      message.to(correo).from(process.env.MAIL_FROM_ADDRESS!).subject('Recuperar contraseña').html(`
  ...
  async reset({ request, response }: HttpContext) {
    ...
    const record = await Database.from('password_resets').where('token', token).first()
    ...
    await Database.from('password_resets').where('correo', record.correo).delete()
